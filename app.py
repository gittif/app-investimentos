...
st.set_page_config(page_title="Investimentos – v3", page_icon="📈", layout="wide")
...
# ---------------------- Helpers: country normalization & FX ----------------------
...
def get_usd_brl():
    try:
        fx = yf.download(['BRL=X'], period="5d", interval="1d", progress=False)['Close'].ffill().iloc[-1]
        return float(fx['BRL=X'] if isinstance(fx, pd.Series) else fx)
    except Exception:
        return np.nan
...
st.title("📊 Controle de Investimentos – v3")
...
with tab2:
    ...
    st.dataframe(fdf[['id','data','ticket','nome','preco','quantidade','valor_investido','compra_venda','onde','tipo','categoria','obs']], use_container_width=True)

    exp1, exp2 = st.columns(2)
    with exp1:
        st.download_button("⬇️ CSV filtrado", data=fdf.to_csv(index=False).encode('utf-8'), file_name="movimentos_filtrado.csv", mime="text/csv")
    with exp2:
...
with tab3:
    ...
    if df.empty:
        ...
        usd_brl = get_usd_brl()
        if 'USA' in by_country_brl['country_norm'].values and not np.isnan(usd_brl):
            usa_brl = by_country_brl.loc[by_country_brl['country_norm']=='USA', 'valor_investido'].iloc[0]
            usa_usd = usa_brl / usd_brl if usd_brl else np.nan
        ...
        if us.empty:
            ...
            if not np.isnan(usd_brl) and not det_us.empty:
...
def fetch_prices(tickers_unique):
    ...
    data = yf.download(tickers_unique, period="5d", interval="1d", progress=False)['Close']
    if isinstance(data, pd.Series):
        data = data.to_frame()
    latest = data.ffill().iloc[-1]
    for tk in tickers_unique:
        try:
            prices[tk] = float(latest[tk])
        except Exception:
            prices[tk] = np.nan
    try:
        fx = yf.download(['BRL=X'], period="5d", interval="1d", progress=False)['Close'].ffill().iloc[-1]
        usd_brl = float(fx['BRL=X'] if isinstance(fx, pd.Series) else fx)
    except Exception:
        usd_brl = np.nan
...
# ---- Editar/Excluir
with tab5:
    st.subheader("Editar ou excluir movimentos")
    df = load_df(conn)
    if df.empty:
        st.info("Sem dados.")
    else:
