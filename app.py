...
st.set_page_config(page_title="Controle de Investimentos – v3.6.5", page_icon="📊", layout="wide")
---------------------- Auth ----------------------
...
Gráfico de pizza por país (tamanho reduzido)
st.markdown("### Alocação por País (BRL)")
if not by_country.empty and by_country['Aporte (BRL)'].sum() > 0:
fig_country, ax_country = plt.subplots(figsize=(4, 4))
...
st.info("Sem dados para gráfico.")
Novo: Por tipo (com conversão)
st.markdown("### Por tipo (com conversão)")
by_tipo_brl = ddf.groupby('tipo', dropna=False)['valor_brl'].sum(min_count=1).reset_index().rename(columns={'valor_brl':'Aporte (BRL)'})
predominante_tipo = ddf.groupby(['tipo','moeda_local'])['valor_local'].sum(min_count=1).reset_index()
...
by_tipo_local = ddf.groupby('tipo')['valor_local'].sum(min_count=1).reset_index().rename(columns={'valor_local':'Aporte (Local)'})
table_tipo = by_tipo_brl.merge(by_tipo_local, on='tipo', how='left').merge(moeda_pred_tipo, on='tipo', how='left')
table_tipo = table_tipo.rename(columns={'moeda_local':'Moeda Local (predominante)'})
table_tipo = table_tipo.sort_values('Aporte (BRL)', ascending=False, na_position='last')
st.dataframe(table_tipo, use_container_width=True)
Gráfico de pizza por tipo (tamanho reduzido, para consistência)
st.markdown("### Alocação por Tipo (BRL)")
if not table_tipo.empty and table_tipo['Aporte (BRL)'].sum() > 0:
fig_tipo, ax_tipo = plt.subplots(figsize=(4, 4))
ax_tipo.pie(table_tipo['Aporte (BRL)'], labels=table_tipo['tipo'], autopct='%1.1f%%', startangle=90)
ax_tipo.axis('equal')
st.pyplot(fig_tipo)
else:
st.info("Sem dados para gráfico.")
