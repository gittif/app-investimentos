...
def insert_movimento(conn, row: dict):
    conn.execute(
        ...
        (data, ticket, nome, preco, quantidade, valor_investido, compra_venda,
...
def build_positions(df: pd.DataFrame, fallback_fx=np.nan):
    ...
    tmp["qtd_signed"] = np.where(
        tmp["compra_venda"] == "Venda", -tmp["quantidade"], tmp["quantidade"]
    )

    agg = (
        tmp.groupby(["ticket", "country"], dropna=False)
        .agg(qtd_total=("qtd_signed", "sum"), aporte=("valor_investido", "sum"))
        .reset_index()
    )

    compras = (
        tmp[tmp["compra_venda"] == "Compra"]
        .groupby("ticket")
        .apply(
            lambda g: (g["preco"] * g["quantidade"]).sum()
            / max(g["quantidade"].sum(), 1e-9)
        )
        .rename("preco_medio")
        .reset_index()
    )
    ...
    positions["moeda"] = np.where(
        positions["country"].str.lower().eq("brasil"),
        "BRL",
        "USD",  # USA e Crypto em USD
    )
    ...
    def to_brl(row):
        if row["moeda"] == "USD" and pd.notna(row["valor_atual_moeda"]) and pd.notna(
            usd_brl
        ):
    ...
    positions["pnl_pct"] = np.where(
        positions["aporte"] != 0,
        positions["pnl_brl"] / positions["aporte"],
        np.nan,
    )
...
with tab1:
    ...
    with col1:
        ...
        preco = st.number_input(
            "Preço por unidade", min_value=0.0, step=0.01, format="%.6f", key="novo_preco"
        )
        quantidade = st.number_input(
            "Quantidade", min_value=0.0, step=1.0, format="%.6f", key="novo_qtd"
        )
    with col2:
        compra_venda = st.selectbox("Operação", ["Compra", "Venda"], key="novo_op")
        onde = st.text_input("Onde (corretora/plataforma)", key="novo_onde")
        tipo = st.text_input(
            "Tipo (ex: Brasil Ações, EUA Ações, FII, ETF)", key="novo_tipo"
        )
        country = st.selectbox(
            "País", ["Brasil", "USA", "Crypto"], index=0, key="novo_pais"
        )
        categoria = st.text_input(
            "Categoria (ex: RV, RF, FII, ETF)", value="RV", key="novo_cat"
        )
        obs = st.text_area("Observações", key="novo_obs")

    ticker_oficial = st.text_input(
        "ticker_oficial (opcional, ex.: XPML11.SA, VOO, BTC-USD)", key="novo_tkoff"
    )
...
with tab2:
    ...
    with c2:
        filtro_ano = st.multiselect(
            "Ano", sorted(df["year"].dropna().unique().tolist()), key="flt_ano"
        )
    with c3:
        filtro_operacao = st.multiselect(
            "Operação", ["Compra", "Venda"], key="flt_op"
        )
    with c4:
    ...
    st.caption(
        "✅ Edite as células (inclui **valor_investido** e **ticker_oficial**), "
        "marque 'Excluir' para remover linhas e use os botões abaixo."
    )

    onde_options = sorted(
        list(set(df["onde"].dropna().tolist() + ["XP", "Rico", "Nubank", "Clear", "Avenue", "Biscoint"]))
    )
    ...
    edited = st.data_editor(
        ...
        column_config={
            ...
            "quantidade": st.column_config.NumberColumn(
                "quantidade", format="%.6f"
            ),
            "valor_investido": st.column_config.NumberColumn(
                "valor_investido",
                format="%.6f",
                help="Pode editar manualmente.",
            ),
            "compra_venda": st.column_config.SelectboxColumn(
                "compra_venda", options=["Compra", "Venda"]
            ),
            "onde": st.column_config.SelectboxColumn("onde", options=onde_options),
            "country": st.column_config.SelectboxColumn(
                "País", options=["Brasil", "USA", "Crypto"]
            ),
            "ticker_oficial": st.column_config.TextColumn(
                "ticker_oficial",
                help="Ticker do Yahoo Finance (ex.: PETR4.SA, VOO, BTC-USD)",
            ),
        },
        ...
        if st.button("💾 Salvar alterações", key="btn_save_inline"):
            ...
            for _, row in merged.iterrows():
                ...
                for col in [
                    ...
                    changed = (pd.isna(new_val) != pd.isna(old_val)) or (
                ...
                changed_inputs = any(
                    k in updates for k in ["preco", "quantidade", "compra_venda"]
                )
                gave_value = ("valor_investido" in updates) and (
                    not pd.isna(to_float_safe(updates["valor_investido"]))
                )
                ...
                if updates:
                    for num_col in ["preco", "quantidade", "valor_investido"]:
                        if (
                            num_col in updates
                            and updates[num_col] is not None
                            and not pd.isna(updates[num_col])
                        ):
    ...
    with colB:
        ids_excluir = (
            edited.loc[edited["excluir"] == True, "id"]
            .dropna()
            .astype(int)
            .tolist()
        )
        if st.button(
            f"🗑️ Excluir selecionados ({len(ids_excluir)})",
            disabled=(len(ids_excluir) == 0),
            key="btn_delete",
        ):
    ...
    with st.expander("Edição em massa de 'onde' (aplica no filtro atual)"):
        novo_onde = st.selectbox(
            "Definir 'onde' para TODAS as linhas filtradas:",
            options=onde_options,
            key="mass_onde",
        )
    ...
    with exp1:
        st.download_button(
            ...
            file_name="movimentos_filtrado.csv",
            mime="text/csv",
            key="dl_csv_filtrado",
        )
    with exp2:
        st.download_button(
            ...
            file_name="movimentos_backup_completo.csv",
            mime="text/csv",
            key="dl_csv_full",
        )
    ...
    if df.empty:
        ...
        with col_fx1:
            ...
            use_manual = st.toggle(
                "Usar cotação manual",
                value=use_manual_default,
                help="Se o yfinance estiver fora, ative e digite a cotação.",
                key="fx_toggle",
            )
        with col_fx2:
            fx_init = float(fx_auto) if pd.notna(fx_auto) else 5.00
            fx_manual = st.number_input(
                "USD/BRL",
                value=fx_init,
                step=0.01,
                format="%.4f",
        ...
        if pd.notna(usd_brl):
            st.caption(
                f"USD/BRL em uso: {usd_brl:,.4f} ({'manual' if use_manual else 'yfinance'})"
            )
        else:
            st.caption(
                "USD/BRL indisponível no momento (defina manualmente para converter os valores de USA)."
            )
        ...
        def split_values(row):
            ...
            if country in ("USA", "Crypto"):
                ...
                return pd.Series(
                    {"valor_local": val, "moeda_local": "USD", "valor_brl": brl}
                )
            else:
                return pd.Series(
                    {"valor_local": val, "moeda_local": "BRL", "valor_brl": val}
                )
        ...
        # --- Por corretora/plataforma
        st.markdown("### Por corretora/plataforma (com conversão)")
        by_onde_brl = (
            ddf.groupby("onde", dropna=False)["valor_brl"]
            .sum(min_count=1)
            .reset_index()
            .rename(columns={"valor_brl": "Aporte (BRL)"})
        )
        predominante = (
            ddf.groupby(["onde", "moeda_local"])["valor_local"]
            .sum(min_count=1)
            .reset_index()
        )
        ...
        by_onde_local = (
            ddf.groupby("onde")["valor_local"]
            .sum(min_count=1)
            .reset_index()
            .rename(columns={"valor_local": "Aporte (Local)"})
        )
        table_onde = (
            by_onde_brl.merge(by_onde_local, on="onde", how="left")
            .merge(moeda_pred, on="onde", how="left")
            .rename(columns={"moeda_local": "Moeda Local (predominante)"})
            .sort_values("Aporte (BRL)", ascending=False, na_position="last")
        )
        st.dataframe(table_onde, use_container_width=True)

        # Gráfico menor por corretora
        st.markdown("#### Alocação por Corretora (BRL)")
        if not table_onde.empty and table_onde["Aporte (BRL)"].sum() > 0:
            col_chart_onde, _ = st.columns([1, 1])
            with col_chart_onde:
                fig_onde, ax_onde = plt.subplots(figsize=(3.5, 3.5))
                ax_onde.pie(
                    table_onde["Aporte (BRL)"],
                    labels=table_onde["onde"],
                    autopct="%1.1f%%",
                    startangle=90,
                )
                ax_onde.axis("equal")
            ...
            st.info("Sem dados para gráfico por corretora.")

        # --- Por país
        st.markdown("### Por país (Local vs BRL)")
        by_country = (
            ddf.groupby("country_norm")
            .agg(
                aporte_local=("valor_local", "sum"),
                aporte_brl=("valor_brl", "sum"),
                moeda=(
                    "moeda_local",
                    lambda x: x.value_counts().index[0] if len(x) > 0 else "BRL",
                ),
            )
            .reset_index()
            .rename(
        ...
        # Gráfico menor por país
        st.markdown("#### Alocação por País (BRL)")
        if not by_country.empty and by_country["Aporte (BRL)"].sum() > 0:
            col_chart_country, _ = st.columns([1, 1])
            with col_chart_country:
                fig_country, ax_country = plt.subplots(figsize=(3.5, 3.5))
                ax_country.pie(
                    by_country["Aporte (BRL)"],
                    labels=by_country["País"],
                    autopct="%1.1f%%",
                    startangle=90,
                )
                ax_country.axis("equal")
                st.pyplot(fig_country, use_container_width=False)
        else:
            st.info("Sem dados para gráfico por país.")

        # --- Por tipo (novo)
        st.markdown("### Por tipo (com conversão)")
        by_tipo_brl = (
            ddf.groupby("tipo", dropna=False)["valor_brl"]
            .sum(min_count=1)
            .reset_index()
            .rename(columns={"valor_brl": "Aporte (BRL)"})
        )
        predominante_tipo = (
            ddf.groupby(["tipo", "moeda_local"])["valor_local"]
            .sum(min_count=1)
            .reset_index()
        )
        idx_tipo = predominante_tipo.groupby("tipo")["valor_local"].idxmax()
        moeda_pred_tipo = predominante_tipo.loc[idx_tipo, ["tipo", "moeda_local"]]
        by_tipo_local = (
            ddf.groupby("tipo")["valor_local"]
            .sum(min_count=1)
            .reset_index()
            .rename(columns={"valor_local": "Aporte (Local)"})
        )
        table_tipo = (
            by_tipo_brl.merge(by_tipo_local, on="tipo", how="left")
            .merge(moeda_pred_tipo, on="tipo", how="left")
            .rename(columns={"moeda_local": "Moeda Local (predominante)"})
            .sort_values("Aporte (BRL)", ascending=False, na_position="last")
        )
        st.dataframe(table_tipo, use_container_width=True)

        # Gráfico menor por tipo
        st.markdown("#### Alocação por Tipo (BRL)")
        if not table_tipo.empty and table_tipo["Aporte (BRL)"].sum() > 0:
            col_chart_tipo, _ = st.columns([1, 1])
            with col_chart_tipo:
                fig_tipo, ax_tipo = plt.subplots(figsize=(3.5, 3.5))
                ax_tipo.pie(
                    table_tipo["Aporte (BRL)"],
                    labels=table_tipo["tipo"],
                    autopct="%1.1f%%",
                    startangle=90,
                )
                ax_tipo.axis("equal")
                st.pyplot(fig_tipo, use_container_width=False)
        else:
            st.info("Sem dados para gráfico por tipo.")

        st.markdown("---")
        st.markdown("### Preços atuais (yfinance) e comparação com preço médio")
        pos_tmp, usd_brl_tmp = build_positions(
            df, fallback_fx=st.session_state.get("fx_in_use", np.nan)
        )
...
with tab4:
    ...
    if df.empty:
        ...
        if pd.isna(fx_fallback):
            fx_fallback = st.number_input(
                "USD/BRL para cálculo das posições (fallback)",
                value=5.00,
                step=0.01,
                format="%.4f",
            )
        ...
        if isinstance(pos, pd.DataFrame) and not pos.empty:
            ...
            k1.metric(
                "Valor total (BRL)",
                f"{pos['valor_atual_brl'].sum(skipna=True):,.2f}",
            )
